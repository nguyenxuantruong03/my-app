...
import { Billboard, Color, Product6, Size } from "@/types";
...
const CategoryPage: React.FC<CategoryPageProps> = ({
  ...
  const [product, setProduct] = useState<Product6[]>([]);
  ...
  const filterProductsByPrice = (products: Product6[]) => {
              ...
              <div className="">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {filterProductsByPrice(sortedProduct).map((item) => (
                    <ProductCard key={item.id} data={item} route="product6" />
                  ))}
                </div>
              </div>
            </div>
