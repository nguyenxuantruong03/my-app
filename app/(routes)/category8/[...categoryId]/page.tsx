...
import { getCategories8 } from "@/actions/categories/get-categories";
import dynamic from "next/dynamic";
import { useEffect, useMemo, useState } from "react";
import { Billboard, Color, Product8, Size } from "@/types";
...
const CategoryPage: React.FC<CategoryPageProps> = ({
  ...
  const [product, setProduct] = useState<Product8[]>([]);
    ...
    const fetchData = async () => {
      try {
        const billboardData = await getBillboard(
          "02e5b508-c778-4399-8e58-8efdef089328"
        );
        const productData = await getProduct8({
              ...
              <div className="">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {sortedProduct.map((item) => (
                    <ProductCard key={item.id} data={item} route="product8" />
                  ))}
                </div>
              </div>
            </div>
