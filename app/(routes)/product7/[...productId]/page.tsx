...
import { getProducts7, getProducts9 } from "@/actions/products/get-products";
...
const ProductPage: React.FC<PropductPageProps> = async ({ params }) => {
  ...
  const product9 = await getProducts9(params.productId);

  const suggestedProducts9 = await getProduct9({
    categoryId: product9?.category?.id,
  });
  ...
  return (
    <div className="bg-white">
      <Container>
        ...
        <ProductListSingleSuggest data={suggestedProducts} route="product7" />
        <h1 className="font-bold text-3xl my-3">Loại khác </h1>
        <ProductListSingleSuggest data={suggestedProducts9} route="product9" />
        <hr className="my-8" />
