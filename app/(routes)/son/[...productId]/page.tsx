...
import { getProducts5, getProducts8 } from "@/actions/products/get-products";
...
const ProductPage: React.FC<PropductPageProps> = async ({ params }) => {
  ...
  const product5 = await getProducts5(params.productId);

  const suggestedProducts5 = await getProduct5({
    categoryId: product5?.category?.id,
  });
  if (!product) {
    return null;
  }
  ...
  return (
    <div className="bg-white">
      <Container>
        ...
        <ProductListSingleSuggest data={suggestedProducts} route="mouse"/>
        <h1 className="font-bold text-3xl my-3">Loại khác </h1>
        <ProductListSingleSuggest data={suggestedProducts5} route="product5" />
        <hr className="my-8" />
