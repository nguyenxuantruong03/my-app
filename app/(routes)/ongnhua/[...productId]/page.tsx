...
import { getProducts2, getProducts3 } from "@/actions/products/get-products";
...
const ProductPage: React.FC<PropductPageProps> = async ({ params }) => {
  ...
  const product3 = await getProducts3(params.productId);

  const suggestedProducts3 = await getProduct3({
    categoryId: product3?.category?.id,
  });
  ...
  return (
    <div className="bg-white">
      <Container>
        ...
        <ProductListSuggest data={suggestedProducts} productType="headphone"/>
        <h1 className="font-bold text-3xl my-3">Loại khác </h1>
        <ProductListSingleSuggest data={suggestedProducts3} route="product3" />
        <hr className="my-8" />
