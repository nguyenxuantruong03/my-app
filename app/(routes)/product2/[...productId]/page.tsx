...
import { getProducts, getProducts2 } from "@/actions/products/get-products";
...
const ProductPage: React.FC<PropductPageProps> = async ({ params }) => {
  ...
  const product0 = await getProducts(params.productId);

  const suggestedProducts0 = await getProduct({
    categoryId: product0?.category?.id,
  });
  ...
  return (
    <div className="bg-white">
      <Container>
        ...
        <ProductListSingleSuggest data={suggestedProducts} route="product2" />
        <h1 className="font-bold text-3xl my-3">Loại khác </h1>
        <ProductListSingleSuggest data={suggestedProducts0} route="product" />
        <hr className="my-8" />
