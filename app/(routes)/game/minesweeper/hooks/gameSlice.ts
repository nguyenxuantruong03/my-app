...
export const slice = createSlice({
  ...
  reducers: {
    generateGrid: (
      state,
      { payload: { width, height, minesCount } }: GenerateGridPayloadAction
    ) => {
      state.grid = [];
    ...
    revealTile: (
      state,
      { payload: startingCoords }: PayloadAction<Coordinates>
    ) => {
      // First reveal loss prevention
      const revealed = revealedCount(state.grid);
      if (
        revealed === 0 &&
        state.grid[startingCoords.y][startingCoords.x].isMine
      ) {
        ...
        for (let y = 0; y < state.grid.length; y++) {
          ...
          for (let x = 0; x < state.grid[y].length; x++) {
            if (
              state.grid[y][x].isMine ||
              (y === startingCoords.y && x === startingCoords.x)
            )
              continue;
            state.grid[y][x].isMine = true;
            relocated = true;
    ...
    incrementTileState: (
      state,
      { payload: { x, y } }: PayloadAction<Coordinates>
    ) => {
...
export const selectTile = (state: RootState, coords: Coordinates) =>
  state.game.grid[coords.y][coords.x];
...
const revealedCount = (grid: Tile[][]) =>
  grid.flat().filter((x) => x.state === "REVEALED").length;
...
export const selectBombRevealed = createSelector(
  [(state: RootState) => state.game.grid],
  (grid) =>
    grid.flat().filter((x) => x.isMine && x.state === "REVEALED").length > 0
);
