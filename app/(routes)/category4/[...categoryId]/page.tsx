...
import { Billboard, Color, Product4, Size } from "@/types";
...
const CategoryPage: React.FC<CategoryPageProps> = ({
  ...
  const [product, setProduct] = useState<Product4[]>([]);
  ...
  const filterProductsByPrice = (products: Product4[]) => {
    ...
    const fetchData = async () => {
      try {
        const billboardData = await getBillboard(
          "5031604f-3245-461a-8670-0b64b5af8aed"
        );
        const productData = await getProduct4({
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {filterProductsByPrice(sortedProduct).map((item) => (
                    <ProductCard key={item.id} data={item} route="product4" />
                  ))}
                </div>
              </div>
            </div>
