...
import { getProducts, getProducts7 } from "@/actions/products/get-products";
...
const ProductPage: React.FC<PropductPageProps> = async ({ params }) => {
  ...
  const product7 = await getProducts7(params.productId);

  const suggestedProducts7 = await getProduct7({
    categoryId: product7?.category?.id,
  });
  ...
  return (
    <div className="bg-white">
      <Container>
        ...
        <ProductListSingleSuggest data={suggestedProducts} route="product" />
        <h1 className="font-bold text-3xl my-3">Loại khác </h1>
        <ProductListSingleSuggest data={suggestedProducts7} route="product7" />
        <hr className="my-8" />
