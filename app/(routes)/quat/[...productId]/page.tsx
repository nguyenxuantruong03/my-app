...
const ProductPage: React.FC<PropductPageProps> = async ({ params }) => {
  ...
  const product4 = await getProducts4(params.productId);

  const suggestedProducts4 = await getProduct4({
    categoryId: product4?.category?.id,
  });
  ...
  return (
    <div className="bg-white">
      <Container>
        ...
        <ProductListSuggest data={suggestedProducts} productType="quat"/>
        <h1 className="font-bold text-3xl my-3">Loại khác </h1>
        <ProductListSingleSuggest data={suggestedProducts4} route="product4" />
        <hr className="my-8" />
